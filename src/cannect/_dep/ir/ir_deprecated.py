...
class IntegrationRequest:
    ...
    def __new__(cls, *models):
        ...
        # Subversion.update(env.SVN.MODEL)
        # Subversion.update(env.SVN.SDD)
        # Subversion.update(env.SVN.CONF)
        # Subversion.update(env.SVN.UNECE)
    ...
    def __init__(self, *models):
        ...
        # 기본 테이블 생성
        self.name = '0000_HNB_SW_IR_.xlsm'
    ...
    @deliverables.setter
    def deliverables(self, path: Union[str, Path]):
        """
        산출물 경로 설정
        """
        self._deliverables = Deliverables(str(path))
    ...
    def commit_all(self, message:str):
        ...
        for row in self:
            ...
            Subversion.commit(self.ws[row["SCMName"].replace('\\', '/')], message)
        return
    ...
    def copy_model_to_svn(
        ...
        for row in self:
            ...
            model = self.ws[row["SCMName"].replace('\\', '/')]
...
if __name__ == "__main__":
    ...
    ir = IntegrationRequest(
        ...
        # "ComDef_HEV", "ComRx_HEV"
        # "CanFDEMSO", "CanFDEMSOM", "CanFDCCUM"
        # "CanFDEMSO_HEV", "CanFDEMSOM_HEV", "CanFDCCUM_HEV",
        "CanEMSOM", "CanEMSO"
    )
    # ir.deliverables = env.DOWNLOADS / 'test'
    ir.deliverables = r'D:\Archive\00_프로젝트\2017 통신개발-\2026\DS0601 CR18734880 OBM 인증 대응(ICE CAN)'
    # ir.deliverables = r'E:\SWF\cannect-api\api\bin\temp\22011148@GUCD24082\ir\0000_HNB_SW_IR_'
