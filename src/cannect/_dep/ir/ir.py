...
class IntegrationRequest(DataFrame):
    ...
    def add_model(self, model: Union[str, Path]):
        ...
        if not name in ["DEve_Typ", "Fid_Typ", "DSig_Typ"]:
            ...
            oid = amd.main["OID"][1:]
            dst = self._path.sdd / f'{self._path.NAME_POST}/{oid}'
            data.SDDName = f'{oid}.zip'

            try:
                svn = SVN.SDD[data.SDDName]
                tools.unzip(svn, self._path.sdd / self._path.NAME_PREV)
                tools.unzip(svn, self._path.sdd / self._path.NAME_POST)
                ...
                with open(str(dst / 'FunctionDefinition.rtf'), 'w') as f:
                    f.write(content.replace("__NAME__", name))
    ...
    def sdd_update(self, user:str, message:str=''):
        if not message:
    ...
    def sync(self, **kwargs):
        ...
        for model in self.index:
            ...
            if not pd.isna(self.loc[model, 'SDDName']):
                ...
                self.loc[model, 'FunctionVersion'] = sdd.ver
                svn = SVN.SDD[self.loc[model, 'SDDName']]
                if not svn is None:
                    self.loc[model, 'SDDRev'] = int(svn.log().loc[0, 'revision'])
...
if __name__ == "__main__":
    ...
    ir = IntegrationRequest("CanFDEMSM14", "ComDef",
    ...
    ir.sdd_update(user=env.USERNAME, message="Hello world")
    ir.sync()
    print(ir)
